        ...
        for (int i = 0; i < wire1.length; i++) {
            ...
            Pair startingPoint = new Pair(x, y);
            wirePoints.add(startingPoint);

            for( int j = 0; j < steps; j++ ) {

                switch (direction) {
                    case "U":
                        wirePoints.add( new Pair<>(x, ++y) );
                        break;
                    case "D":
                        wirePoints.add( new Pair<>(x, --y) );
                        break;
...
//        for (Pair<Integer, Integer> wirePoint : wirePoints) {
//            System.out.println(wirePoint.getKey());
//        }
        ...
        for (int i = 0; i < wire2.length; i++) {
            ...
            for (int j = 0; j < steps; j++) {
                ...
                for (Pair<Integer, Integer> wirePoint : wirePoints) {
                    if(wirePoint.getKey() == x && wirePoint.getValue() == y) {
                        intersections.add(wirePoint);
                    }
                }
        ...
        for (Pair<Integer, Integer> intersection : intersections) {
            distances.add(Math.abs(intersection.getKey()) + Math.abs(intersection.getValue()));
        }
