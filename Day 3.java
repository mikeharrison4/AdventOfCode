...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // FIND THE INTERSECTION THAT TAKES THE FEWEST STEPS TO GET TO
            // for each movement, need to incrememnt a counter for both wires. Need to find a way of passing in the counter for wire 1 so can use it when matched
...
//        List<Pair<Integer, Integer>> wirePoints = new ArrayList<>();
        ...
        List<Integer> counterSteps = new ArrayList<>();


        // Starting coordinates (not sure if i need these)
        int x = 0;
        int y = 0;
        ...
        for (int i = 0; i < wire1.length; i++) {
            String direction = wire1[i].substring(0, 1);
            int steps = Integer.parseInt(wire1[i].substring(1));
            Map<String, Integer> startingPoint = new HashMap<>();
            startingPoint.put(x + "," + y, 0);
            ...
            for( int j = 0; j < steps; j++ ) {
                ...
                switch (direction) {
                    case "U":
                        wirePoints.put(x + "," + ++y, ++counterSteps1);
                        break;
                    case "D":
                        wirePoints.put(x + "," + --y, ++counterSteps1);
                        break;
        ...
        for (int i = 0; i < wire2.length; i++) {
            String direction = wire2[i].substring(0, 1);
            int steps = Integer.parseInt(wire2[i].substring(1));
            ...
            for (int j = 0; j < steps; j++) {
                ...
                if(wirePoints.containsKey(x + "," + y)) {
                    intersections.add(x + "," + y);
                    counterSteps.add(wirePoints.get(x + "," + y) + counterSteps2);
                }
        ...
        Collections.sort(counterSteps);

        for (Integer integer : counterSteps) {
        ...
        for (String intersection : intersections) {
            String[] intersectionSplit = intersection.split(",");
            int xCoord = Integer.parseInt(intersectionSplit[0]);
            int yCoord = Integer.parseInt(intersectionSplit[1]);
            distances.add(Math.abs(xCoord) + Math.abs(yCoord));
        }

        Collections.sort(distances);
        int closestDistance = distances.get(0);
